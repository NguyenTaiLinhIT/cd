...
namespace _3_PL
{
    public partial class frmQUANLYNHACC : Form
    {
        ...
        INhaCungCapCTServices chiTietNCCServives;
        ...
        public frmQUANLYNHACC()
        {
            ...
            chiTietNCCServives = new NhaCungCapCTService();
        ...
        private void dataGridView1_CellClick_2(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //// Đổ dữ liệu lên dataGridView2
            //dataGridView2.DataSource = products;
            var idhd = Guid.Parse(dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString());
            var hdct = from x in chiTietNCCServives.GetAll()
                       join y in SanPhamServices.GetAll() on x.ProductId equals y.Id
                       join z in _qlNhacc.GetAll() on x.SupplierId equals z.Id
                       where z.Id == idhd
                       select new
                       {
                           MaSanPham = y.Id,
                           TenSanPham = y.Name,
                           GiaBan = y.Price,
                           SoLuongTon = y.Quantity
                       };
            dataGridView2.Rows.Clear();
            dataGridView2.Columns[0].Visible = false;
            foreach (var x in hdct)
            {
                dataGridView2.Rows.Add(x.MaSanPham, x.TenSanPham, x.GiaBan, x.SoLuongTon);
            }
        }
